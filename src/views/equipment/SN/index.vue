    ...
    <a-card style="border-radius: 4px; margin-top: 10px">
      ...
      >
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'production_category'">
            ...
            <span v-if="record.production_category">
              （{{ record.production_category }}）</span
            >
          </template>
          <template v-if="column.key === 'production_model'">
            <span>{{ record.production_model_cn }}</span>
            <span v-if="record.production_model"
              >（{{ record.production_model }}）</span
            >
          </template>
      ...
      <div style="margin-top: 10px; text-align: right">
        <a-pagination
          ...
          v-model:current="pageData.page_index"
          v-model:page-size="pageData.page_size"
          :total="listTotal"
          :show-total="(listTotal) => `共 ${listTotal} 条数据`"
          @change="changePage"
        />
      </div>
    </a-card>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getBatchDataSource = () => {
      getBatchList({ ...pageData, ...formState })
        .then((res) => {
          if (res.code == 0) {
