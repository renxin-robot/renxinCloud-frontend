<template>
    <div class="ele-body">
        ...
        <a-modal v-model:visible="showAssistantVisible" title="店员列表" @ok="handleOk" width="900px" :footer="null">
            <div>
                <div v-for="item,index in assistantList" :key="item.id"
                    ...
                    <div style="display: flex; align-items: center;">
                        ...
                        <div style="display: flex; align-items: center; margin-right: 20px">
                            ...
                            <div style="width: 180px;" v-if="editAssistantIndex!==index">
                                {{ item.name }}
                            </div>
                        ...
                        <div style="display: flex; align-items: center; margin-right: 20px">
                            ...
                            <div style="width: 180px;" v-else>
                                <a-input v-model:value="commissionData.password" placeholder="请输入密码..." />
                            </div>
                        </div>
                    </div>
                    <div style="display: flex;">
                        <div v-if="editAssistantIndex!==index">
                            <a-button type="link" @click="editAssistant(item,index)" >编辑</a-button>
                        </div>
                        <div v-else style="display: flex;">
                            <a-button type="link" @click="cancleEditAssistant(index)">确定</a-button>
                            <a-button type="link" @click="cancleEditAssistant(index)"><span style="color:gray;">取消</span></a-button>
                        </div>
                        <div>
            ...
            <div style="display: flex; align-items: center">
                <div style="display: flex; align-items: center; margin-right: 20px">
                    <div style="margin-right: 10px">角色</div>
                    <div>
                        <a-select style="width: 90px" ref="select" v-model:value="commissionData.role">
                            <a-select-option value="店长">店长</a-select-option>
                            <a-select-option value="厨师">厨师</a-select-option>
                            <a-select-option value="操作员">操作员</a-select-option>
                        </a-select>
                ...
                <div style="display: flex; align-items: center; margin-right: 20px">
                    <div style="margin-right: 10px">账号</div>
                    <div>
                        <a-input v-model:value="commissionData.account" placeholder="请输入账号..." />
                    </div>
                </div>
                <div style="display: flex; align-items: center; margin-right: 20px">
                    <div style="margin-right: 10px">密码</div>
            ...
            <div style="display: flex; justify-content: center; margin-top: 200px">
                <a-button style="margin-right: 80px">取消</a-button>
...
export default defineComponent({
    ...
    setup() {
        ...
        let editAssustantInfo=reactive({})
        let areaList = ref([]);
        ...
        const addStoreAssistant = (row) => {
            ...
            getUserEmployee(row.id).then((res) => {
                ...
                console.log(assistantList.value);
            });
        };

        const editAssistant=(row,index)=>{
            editAssistantIndex.value=index
            Object.assign(editAssustantInfo,row)
            console.log(row,index)
        }

        const cancleEditAssistant=(index)=>{
