<template>
    <a-card>
        ...
        <template #default="{ node, data }">
            <span>
            <el-checkbox v-model="node.checked" @change="handleCheckAllChange"></el-checkbox>
            {{ node.label }}
            </span>
        </template>
        </el-tree>
...
export default defineComponent({
    setup() {
        ...
        const handleCheckAllChange = (val) => {
            ...
            if (val) {
            //全选
            this.$refs.permListRef.setCheckedNodes(getPermissionList(data.value));
            }else{
                //取消选中
                this.$refs.permListRef.setCheckedKeys([]);
            }
            // defaultCheckedNode.value = val ? getPermissionList(data.value): []
            // console.log(defaultCheckedNode.value)
            // }
