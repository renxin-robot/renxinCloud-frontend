            ...
            <a-form :model="customerData" name="basic" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }" autocomplete="off">
                <a-form-item label="佣金方案" name="commission_plan_id"
                    ...
                    v-model:value="customerData.commission_plan_id">
                        <a-select-option :value="item" v-for="item in commissionPlanList" :key="item.id">{{item.name}}</a-select-option>
                    </a-select>
                </a-form-item>
                <a-form-item label="订单金额"
                    :rules="[{ required: true, message: '请选择佣金方案！' }]">
                    <a-input v-model:value="orderPrice" placeholder="" />
                </a-form-item>
                <a-form-item label="设备台账" name="device_journal_id"
                    :rules="[{ required: true, message: '请选择设备台账！' }]">
                    <a-select
                    ref="select"
                    v-model:value="customerData.device_journal_id">
