            ...
            <div id="proTable">
                ...
                <template #bodyCell="{ column, record }">
                    <template v-if="column.key === 'status'">
                        ...
                        <a-tag v-else-if="record?.status == 'paid'" color="blue">已支付</a-tag>
...
export default defineComponent({
    ...
    setup() {
        ...
        const selectLoadMoreSnInfo = async () => {
          ...
          // 获取10条数据
            deviceJournalList.value = [...(deviceJournalList.value.filter((item)=>{
                return item.status!='在库'
            })), ...((await getStoreInformation(page.value++, limit.value)).filter((item)=>{
                return item.status!='在库'
            }))]
            
            console.log(deviceJournalList.value,'hihuih')
        }
    ...
    created() {
        (async () => {
            this.deviceJournalList = (await this.getStoreInformation(1, 10)).filter((item)=>{
                console.log(item)
                return item.status!='在库'
