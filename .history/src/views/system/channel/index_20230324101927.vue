<template>
    <div class="ele-body">
        ...
        <a-card style="margin-top: 10px;border-radius: 4px;">
            <ele-pro-table ref="tableRef" title="渠道列表" :resizable="true" :bordered="true" :height="tableHeight"
                :full-height="fixedHeight ? 'calc(100vh - 168px)' : void 0" :columns="columns" :datasource="datasource"
                :custom-row="customRow" :scroll="{ x: 1000 }" cache-key="proListBasicTable" @done="onDone">
                <!-- 表头工具按钮 -->
                <template #toolkit>
        ...
        <a-modal v-model:visible="addVisible" title="新增渠道" @ok="handleOk">
            <a-form :model="channelData" name="basic" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }" autocomplete="off"
...
import { getChannel, deleteChannel } from '@/api/system/channel';
