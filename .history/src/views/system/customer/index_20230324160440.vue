    ...
    setup() {
        ...
        const getAreaList=()=>{
            getArea().then((res)=>{
                ...
                let allValues = Object.values(res.data)
                let allKeys = Object.keys(res.data)
                let arr={}
                let childArr=[]
                allValues.map((child)=>{
                    let childKeys=Object.keys(child)
                    let childValues=Object.values(child)
                    childKeys.forEach( (item, index) => {
                        let params = {
                            label: item,
                            value: childValues[index],
                            // children:res.data[item]
                        }
                        childArr.push(params)
                    })
                })
                ...
                areaList.value=stateList.value.map((item,index)=>{
                    return{
                        ...item,
                        children:arr[item.label]
                    }
                })
                // areaList.value=res.data.map
                console.log(areaList.value,res.data)
            })
        }
        getAreaList()
        return {
