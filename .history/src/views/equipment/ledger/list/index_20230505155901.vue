            ...
            <div style="display: flex;justify-content: space-between;">
                <el-form :inline="true" :model="formState" class="demo-form-inline">
                    <el-form-item label="设备编号">
                        <a-input v-model:value="formState.category" placeholder="请输入设备编号"/>
                    </el-form-item>
                    <el-form-item label="设备类型">
                        <a-input v-model:value="formState.category" placeholder="请输入设备类型"/>
                    </el-form-item>
                    <el-form-item label="设备型号">
                        <a-input v-model:value="formState.category" placeholder="请输入设备型号"/>
                    </el-form-item>
                </el-form>
                <div>
                ...
                <ele-pro-table ref="tableRef" title="设备列表" :resizable="true" :bordered="true" :columnsFixed="true"
                    ...
                    <template #bodyCell="{ column, record }">
                        <template v-if="column.dataIndex === 'device_code'">
                            <router-link to="/equipment/ledger/list/detail">{{ record.device_code}}</router-link>
                        </template>
                        <template v-if="column.dataIndex === 'name'">
                            {{ record.device_info.name}}
                        </template>
                        <template v-if="column.dataIndex === 'type'">
                            {{ record.device_info.type}}
                        </template>
                        ...
                        <template v-else-if="column.key === 'action'">
                            ...
                            <a-divider type="vertical" />
                                <a-tooltip placement="bottom" color="white">
                                    <template #title>
                                        <div>
                                            <div style="padding: 5px;"><a>布机</a></div>
                                            <div style="padding: 5px;"><a>移机</a></div>
                                            <div style="padding: 5px;"><a>撤机</a></div>
                                        </div>
                                    </template>
...
<script>
import { defineComponent, reactive, ref, computed ,created} from 'vue'
...
export default defineComponent({
    ...
    setup() {
        ...
        const getDeviceJournalList = () => {
            getDeviceJournal({ ...pageData }).then((res) => {
                console.log(res)
                if (res.code == 0) {
