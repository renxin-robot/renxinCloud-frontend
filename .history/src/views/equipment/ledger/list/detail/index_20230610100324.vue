...
export default defineComponent({
    ...
    setup(){
        ...
        const detailInfo=JSON.parse(localStorage.getItem('detail'))
        let activeKey=ref('1')
        const  getCookedInfo=()=>{
            getCooked(detailInfo.device_code).then((res)=>{
                if(res.code==0){
                    cookedList.value=res.data
                }
                var times = (new Date(1686110423983)).toLocaleDateString().replace(/\//g,"-")+" "+(new Date(1686110423983)).toTimeString().substr(0,8);
                console.log(times)
            })
        }
        getCookedInfo()
