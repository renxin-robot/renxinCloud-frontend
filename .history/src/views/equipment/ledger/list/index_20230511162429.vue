                        ...
                        <template v-else-if="column.key === 'action'">
                            ...
                            <a-divider type="vertical" />
                                <a-tooltip placement="bottom" color="white">
                                    <template #title>
                                        <div>
                                            <div style="padding: 5px;"><a @click="toDeployment(record)">布机</a></div>
...
export default defineComponent({
    ...
    setup() {
        ...
        const toDeployment=(row)=>{
            push({
                name:'deployment',
                query:{code:row.device_code,status:row.status,name:row.device_info.name,type:row.device_info.type,category:row.device_info.category,id:row.device_id}
            })
        }
        return {
            toDeployment,
