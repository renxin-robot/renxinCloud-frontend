    ...
    <div class="ele-body">
        ...
        <a-card style="margin-top: 10px;border-radius: 4px;">
            ...
            <div style="text-align:right;margin-top: 10px;">
                <a-pagination size="small" :total="nameplateCount" @change="changePage" :show-total="nameplateCount => `共 ${nameplateCount} 条`" />
            </div>
        </a-card>
        <a-modal v-model:visible="addVisible" :title="`${editId?'编辑':'新增'}铭牌`" @ok="handleOk">
            <a-form :model="namePlateInfo" name="basic" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }" autocomplete="off">
                <!-- <a-form-item label="铭牌名称" name="name"
                    :rules="[{ required: true, message: '请输入铭牌名称！' }]">
                    <a-input v-model:value="namePlateInfo.name" />
                </a-form-item> -->
                <a-form-item label="出厂编号" name="code"
                    :rules="[{ required: true, message: '请输入出厂编号！' }]">
                    <a-input v-model:value="namePlateInfo.code" />
                </a-form-item>
                <a-form-item label="屏幕编号" name="screen_code"
                    :rules="[{ required: true, message: '请输入屏幕编号！' }]">
...
import {getNameplate,deleteNameplate} from '@/api/equipment/basic/nameplate'
