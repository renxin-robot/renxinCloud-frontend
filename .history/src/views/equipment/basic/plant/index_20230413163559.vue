    ...
    <div class="ele-body">
            ...
            <div style="display: flex;justify-content: space-between;">
                <el-form :inline="true" :model="formState" class="demo-form-inline">
                    <el-form-item label="出厂编号">
                        <a-input v-model:value="formState.code" placeholder="请输入出厂编号" @pressEnter="toSearch" />
                    </el-form-item>
                    <el-form-item label="屏幕编号">
                        <a-input v-model:value="formState.screen_code" placeholder="请输入屏幕编号" @pressEnter="toSearch" />
                    </el-form-item>
                </el-form>
                <div>
        ...
        <a-modal v-model:visible="addVisible" :title="`${ editId ? '编辑' : '新增' }铭牌`" @ok="handleOk">
            <a-form :model="plantInfo" name="basic" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }"
                ...
                <a-form-item label="生产商编号" name="code" :rules="[{ required: true, message: '请输入出厂编号！' }]">
                    <a-input v-model:value="plantInfo.code" />
                </a-form-item>
                <a-form-item label="生产商地址" name="address" :rules="[{ required: true, message: '请输入屏幕编号！' }]">
                    <a-input v-model:value="plantInfo.address" />
                </a-form-item>
                <a-form-item label="生产商电话" name="telphone" :rules="[{ required: true, message: '请输入屏幕编号！' }]">
...
import { deleteNameplate, addNameplate } from '@/api/equipment/basic/nameplate'
import {getFactory,addFactory} from '@/api/equipment/basic/ plant'
export default defineComponent({
    ...
    setup() {
        ...
        const toAdd = () => {
            // addNameplate({...plantInfo}).then((res)=>{
