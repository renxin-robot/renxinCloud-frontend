        ...
        <a-modal v-model:visible="addVisible" :title="`${ editId ? '编辑' : '新增' }产品`" @ok="handleOk">
            <a-form :model="productInfo" name="basic" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }"
                ...
                <a-form-item label="备注" name="remark"
                    ...
                    <a-textarea v-model:value="productInfo.remark" placeholder="Basic usage" :rows="4" />
                </a-form-item>
            </a-form>
        </a-modal>
    </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        const editChannel = (row) => {
            editId.value = row.id
            productInfo.code = row.code
            productInfo.name = row.name
            productInfo.telphone = row.telphone
            productInfo.address = row.address
        ...
        const handleOk = () => {
            if(editId.value){
                ...
                addFactory(productInfo).then((res) => {
                    if (res.code ==0) {
                        notification.success({
                            message: '新增成功',
                        });
